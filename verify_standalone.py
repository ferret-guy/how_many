...
def check_paths():
    ...
    if not SCIPY_REPO_PATH.is_dir():
        pytest.fail(
            f"SciPy repo not found at: {SCIPY_REPO_PATH}\n"
...
def test_find_peaks_scipy_original(tmp_path):
    ...
    result = run_tests_from_copy(
        PEAK_TEST_FILE, "TestFindPeaks", tmp_path, "peaks_original"
    )
    assert (
...
def test_find_peaks_standalone(tmp_path):
    ...
    with patch("scipy.signal.find_peaks", new=standalone.find_peaks):
        result = run_tests_from_copy(
            PEAK_TEST_FILE, "TestFindPeaks", tmp_path, "peaks_standalone"
        )
...
def test_detrend_scipy_original(tmp_path):
    ...
    result = run_tests_from_copy(
        DETREND_TEST_FILE, "TestDetrend", tmp_path, "detrend_original"
    )
...
def test_detrend_standalone(tmp_path):
    ...
    with patch("scipy.signal.detrend", new=standalone.detrend):
        result = run_tests_from_copy(
            DETREND_TEST_FILE, "TestDetrend", tmp_path, "detrend_standalone"
        )
